...
export class AssetService {
  ...
  public GetAssets(): Array<AssetInfo> {
    return this.GetAllAssets().filter(a => a.AssetStatus != "Blockup");
  }
  ...
  public Load(): Promise<Array<AssetInfo>> {
    return new Promise<Array<AssetInfo>>((resolve, reject) => {
      this.Assets = [];
      let id = this.http.get(this.Url).subscribe((response: { IsSuccess: boolean, Data: any[] }) => {
        if (response && response.IsSuccess) {
          this.Assets = response.Data;
